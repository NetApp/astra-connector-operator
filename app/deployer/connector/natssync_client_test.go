...
func TestNatsSyncGetDeploymentObjectsDefault(t *testing.T) {
	...
	assert.Equal(t, "netappdownloads.jfrog.io/docker-astra-control-staging/arch30/neptune/natssync-client:2.1.202309262120", deployment.Spec.Template.Spec.Containers[0].Image)
	assert.Equal(t, "192.168.1.1", deployment.Spec.Template.Spec.HostAliases[0].IP)
