...
step_apply_trident_operator_patches() {
    ...
    if [ "$patches_len" -eq 0 ]; then
        logdebug "no trident operator patches to apply, skipping"
        return 0
...
step_apply_torc_patches() {
    ...
    if [ "$patches_len" -eq 0 ]; then
        logdebug "no TORC patches to apply, skipping"
        return 0
    elif ! trident_will_be_installed_or_modified; then
...
if [ "$_RESOURCES_OR_PATCHES_HAVE_BEEN_APPLIED" != "true" ]; then
    ...
    _msg+=" state and/or all applicable operations were declined by the user."
    loginfo "$_msg"
